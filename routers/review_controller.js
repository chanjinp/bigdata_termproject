...
// review_router.post('/writeReview/:reservation_id', async (req, res) => {
//     try {
//         // guest의 이름을 찾음
//         const guest = await Guest.findOne({name: req.body.guest_name});
//
...
review_router.post('/:reservation_id', async (req, res) => {
    try {
        ...
        if (!reservation) {
            return res.status(400).json({ message: '예약을 찾을 수 없습니다.' });
